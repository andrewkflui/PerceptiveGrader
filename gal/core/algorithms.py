...
class GAL(Algorithm):
	def __init__(self, subspace_param_list, possible_grades, default_grade=None,
		grading_actions=None, batch_sizes=None, random_seed=None, **kwargs):
		...
		# Spaciousness, RD and Delta-link threshold parameters
		...
		self.random_answer_selection = False

		# self.confidence_version = 'ig_norden'
		self.confidence_version = None
		self.entropy_calculation_method = None	# None (= information gain) / simple
		self.rd_cutoff_adjustment_factor = None
		self.delta_link_threshold_adjustment_factor = None
		self.density_log_base = None
		self.entropy_density_weighting_method = None
		### end of internal configrations
		...
		self.rd_cutoff_adjustment = self.delta_link_threshold_adjustment = 0
		### End of user adjustable parameters

		self.default_rd_cutoff = self.default_delta_link_threshold = None
		...
		### end of version < 6 configurations
	...
	def get_printable_params(self):
		params = {
			...
			'Grading Actions': self.get_grading_actions(),
		...
		params['RD Cutoff Adjustment Factor'] = self.rd_cutoff_adjustment_factor
		params['Delta Link Threshold Adjustment Factor'] = self.delta_link_threshold_adjustment_factor
		params['Density Log Base'] = self.density_log_base
		params['Entory Density Weighting Method'] = self.entropy_density_weighting_method
	...
	def adjust_cutoff(self, cutoff_type, adjustment):
		if cutoff_type == 'rd_cutoff':
			self.rd_cutoff_adjustment = adjustment
			self.rd_cutoff = self.default_rd_cutoff + adjustment
		elif cutoff_type == 'delta_link_threshold':
			self.delta_link_threshold_adjustment = adjustment
			self.delta_link_threshold = self.default_delta_link_threshold + adjustment
		for subspace in self.subspaces:
			subspace.adjust_cutoff(cutoff_type, adjustment)
		self.update_representations(update_cutoffs=False)

	def get_grading_actions(self):
		return self.grading_actions or len(self.reduced_known_data_labels)
	...
	def run(self, batch_size=None):
		try:
			if self.grading_actions is None and batch_size is None:
				raise utils.ParamNotSpecifiedError(params=['batch_size'])
			if (self.iterations is not None and self.current_iteration > self.iterations - 1) or not batch_size:
				return [], [], None, None, None, None, None, None, None, None
			assigned_grading_actions = batch_size or self.batch_sizes[self.current_iteration]
	...
	def mark_answers(self, answer_dict):
		...
		for key, value in self.known_data_labels.items():
			index = int(self.original_to_reduced_index_map[key])
	...
	def add_data(self, new_data_list, additional_grading_actions=0, additional_batch_sizes=[]):
		if self.grading_actions is not None and additional_grading_actions is not None:
		...
		for s in range(len(self.subspaces)):
			params, subspace = self.subspace_param_list[s], self.subspaces[s]
			if subspace.random_selections is None:
				new_data = np.array(new_data_list[subspace.encoder])
			else:
				random_reference_positions = params.get('random_reference_positions', [(0, 1)])	
				random_selection_method = subspace.random_selection_method
				horizontal_random_selection_number = subspace.horizontal_random_selection_number

				# new_data = None
				# for r in range(len(random_reference_positions)):
				# 	random_reference_position, proportion = random_reference_positions[r]
				# 	random_selections = subspace.random_selections[r]
				# 	reference_subspace = self.subspaces[random_reference_position]
				# 	# d = np.copy(new_data_list[reference_subspace.encoder])
				# 	# if random_selection_method == 'vertical':
				# 	# 	d = np.transpose(np.transpose(d)[random_selections])
				# 	# else:
				# 	# 	d = d[random_selections]
				# 	self.get_data_from_random_dimensions(new_data_list[reference_subspace.encoder],
				# 		random_selections, random_selection_method)
				# 	new_data = np.transpose(d) if new_data is None else np.append(new_data, np.transpose(d), 0)
				# new_data = np.transpose(new_data)

				# if random_selection_method == 'horizontally':
				# 	# fit to pca and transform
				# 	pca = decomposition.PCA(n_components=subspace.data_dim, svd_solver='full')
				# 	new_data = pca.transform(self.subspaces[random_reference_positions[0][0]])
					
				# 	# svd = decomposition.TruncatedSVD(n_components=random_dimension)
				# 	# svd.fit(selected_data)
				# 	# data = svd.transform(data)

				new_data, _ = self.generate_subspace_data(random_reference_positions,
					random_selection_method, new_data_list=new_data_list,
					random_selections=subspace.random_selections)
			
			if s == 0:
				subspace.add_data(new_data, encapsulate=True)
				self.update_reduced_data_indices(subspace)
			else:
				subspace.add_data(new_data, encapsulate=False,
					reduced_data_indices=self.reduced_data_indices,
					reduced_data_index_dict=self.reduced_data_index_dict)
	...
	def init_subspaces(self, subspace_param_list):
		...
		if self.reset_subspaces:
			...
			for s in range(len(subspace_param_list)):
				# hot fix
				self.subspaces[s].entropy_calculation_method = None
				self.subspaces[s].rd_cutoff_adjustment_factor = None
				self.subspaces[s].delta_link_threshold_adjustment_factor = None
				self.subspaces[s].density_log_base = None
				self.subspaces[s].entropy_density_weighting_method = None
		...
		self.update_representations()
		self.default_rd_cutoff = self.rd_cutoff
		self.default_delta_link_threshold = self.delta_link_threshold
	...
	def select_random_dimensions(self, random_dimension, horizontal_random_selection_number,
		...
		while True:
			...
			for subspace in self.subspaces:
				...
				if subspace.random_selections is not None \
					and subspace.random_selection_method == random_selection_method \
					and len(set(reduce(lambda a, b: a + b, subspace.random_selections)) - set(selected)) <= 0:
					duplicated = True
					break
			if not duplicated:
		...
		data = self.get_data_from_random_dimensions(reference_subspace.data, random_selections,
			random_selection_method)

		return data, random_selections

	def get_data_from_random_dimensions(self, source_data, random_selections, random_selection_method):
		data = np.copy(source_data)
		if random_selection_method == 'vertical':
			data = np.transpose(np.transpose(data)[random_selections])
		else:
			data = data[random_selections]
	...
	def generate_subspace_data(self, random_reference_positions, random_selection_method,
		random_dimension=None, horizontal_random_selection_number=None, new_data_list=None,
		random_selections=None):
		data = None
		# for random_reference_position, proportion in random_reference_positions:
		for i in range(len(random_reference_positions)):
			random_reference_position, proportion = random_reference_positions[i]
			reference_subspace = self.subspaces[random_reference_position]
			...
			if random_dimension is not None:
				...
				# d = self.get_data_from_random_dimensions(new_data_list[reference_subspace.encoder],
				# 	random_selections, random_selection_method)
				source_data = new_data_list[reference_subspace.encoder] \
					if random_selection_method == 'vertical' \
					else self.subspaces[random_reference_position].data
				d = self.get_data_from_random_dimensions(source_data, random_selections[i],
					random_selection_method)
		...
		if random_selection_method == 'horizontally':
			...
			pca.fit(data)
			# data = pca.transform(self.subspaces[random_reference_positions[0][0]])
			if new_data_list is None:
				data = pca.transform(self.subspaces[0].data)
			else:
				data = pca.transform(new_data_list[self.subspaces[0].encoder])
	...
	def create_subspace(self, params):
		...
		if 'random_dimension' in params:
			...
			data, random_selections = self.generate_subspace_data(random_reference_positions, random_selection_method,
				random_dimension=random_dimension, horizontal_random_selection_number=horizontal_random_selection_number)
			params['random_selections'] = random_selections
	...
	def update_batch_size(self):
		if self.grading_actions is None:
			self.batch_sizes = []
			return
	...
	def update_representations(self, update_cutoffs=True):
		...
		self.averaged_spaciousness = None
		...
		if update_cutoffs:
			...
			if self.grade_assignment_method == 'parent_breaks':
				self.delta_link_threshold = np.mean([s.delta_link_threshold * s.weight for s in self.subspaces if s.weight > 0])
			else:
	...
	def get_valuable_answer_indices(self, assigned_grading_actions):
		# baseline
		if self.random_answer_selection:
			unassigned_indices = [i for i in range(self.reduced_num_data) \
				if i not in self.reduced_known_data_labels]
			indices = []
			while len(unassigned_indices) > 0 and len(indices) < assigned_grading_actions:
				index = random.choices(population=unassigned_indices)[0]
				unassigned_indices.remove(index)
				indices.append(index)

			if self.remaining_grading_actions is not None:
				self.remaining_grading_actions -= len(indices)
			else:
				 self.batch_sizes.append(assigned_grading_actions)
			self.current_iteration += 1

			zeros_scalar = np.zeros(shape=self.reduced_num_data)
			zeros_2d = np.zeros(shape=(len(self.subspaces), self.reduced_num_data))
			return self.get_original_data_indices(indices, first_only=True), indices, \
				None, zeros_scalar, zeros_scalar, zeros_2d, zeros_2d, zeros_2d, zeros_scalar, []
	...
	def calculate_densities(self):
		...
		# self.densities = np.log(self.densities)
		if self.density_log_base is not None:
			self.densities = np.log10(self.densities) / np.log10(self.density_log_base)
		else:
			self.densities = np.log(self.densities)
	...
	def calculate_spaciousness(self):
		...
		# elif self.version == 3 or self.version == 4:
			...
			self.spaciousness = self.calculate_weighted_average_subspace_values(
				[s.normalized_spaciousness for s in self.subspaces])
			self.averaged_spaciousness = self.calculate_weighted_average_subspace_values(
				[s.spaciousness for s in self.subspaces])
		else:
			...
			self.averaged_spaciousness = np.mean([s.spaciousness for s in self.subspaces], axis=0)
	...
	def calculate_global_marginalness(self, label_list):
		...
		keys = list(set(keys))
		weights = [self.possible_grades_weights[self.possible_grades.index(key)] for key in keys]
		...
		for i in range(self.reduced_num_data):
			...
			if not self.weighted_global_entropy:
				...
				# 	u_g[i] += self.possible_grades_weights[self.possible_grades.index(keys[k])] * p * math.log2(p)
				# u_g[i] *= -1
				u_g[i] = -1 * sum([probabilities[i][k] * math.log2(probabilities[i][k]) * weights[k] \
					for k in range(len(keys)) if probabilities[i][k] != 0])
		return u_g
