...
class GALSubspace():
	def __init__(self, data, encoder, model, possible_grades, default_grade, weight=1, **kwargs):
		self.id = None
		self.data = data
		...
		self.entropy_calculation_method = None
		...
		self.cutoff_adjusted_callback = None
	...
	def get_name(self):
		...
		# 	name = 'GUSE'
		if 'google_universal_sentence_encoder' in self.encoder:
			name = self.encoder.replace('google_universal_sentence_encoder', 'GUSE')
		else:
	...
	# 		self.rd_cutoff += self.rd_cutoff_adjustment
	# 	elif cutoff_type == 'delta_link_threshold':
	# 		self.delta_link_threshold_adjustment = adjustment
	# 		self.delta_link_threshold += self.delta_link_threshold_adjustment
	# 	self.calculate_densities()
	# 	if self.cutoff_adjusted_callback is not None:
	# 		self.cutoff_adjusted_callback()
	...
	def update_representations(self):
		...
		if self.default_rd_cutoff is None:
			self.default_rd_cutoff = self.rd_cutoff
		if self.default_delta_link_threshold is None:
			self.default_delta_link_threshold = self.delta_link_threshold
		self.calculate_densities()
		# self.calculate_spaciousness()
		...
		if version == 1:
			...
			if version == 2:
				...
				if self.density_log_base is not None:
					return math.log(dw, 10) / math.log(self.density_log_base) + 1
				else:
					return math.log(dw) + 1
			else:
	...
	def add_data(self, new_data, encapsulate=True, reduced_data_indices=None,
		reduced_data_index_dict=None):
		...
		self.normalized_densities = np.append(self.normalized_densities, np.zeros(shape=new_data_len))
		self.data_distances = utils.get_data_distances(self.data, self.distance_function)
		...
		if encapsulate:
			...
			self.set_reduced_data(reduced_data_indices, reduced_data_index_dict)
		...
		if self.rd_cutoff is None and self.rd_deriving_factor is None:
			...
			if self.rd_cutoff is not None:
				for s in range(spaciousness.size):
					if spaciousness[s] >= self.rd_cutoff:
						...
						break
				rd_cutoff = self.rd_cutoff
			else:
				# self.rd_cutoff_position = int(ranks[-1] / 2)
				self.rd_cutoff_position = int(ranks[-1] * self.rd_deriving_factor)
				rd_cutoff = spaciousness[self.rd_cutoff_position]

			if self.rd_cutoff_adjustment_factor is not None:
				rd_cutoff *= self.rd_cutoff_adjustment_factor

			# find delta_link_threshold
			if self.grade_assignment_method == 'parent_breaks' and self.delta_link_threshold is None:
				if self.delta_link_threshold_factor is None:
					...
					self.delta_link_threshold = rd_cutoff * self.delta_link_threshold_factor
					self.delta_link_threshold_window_size = self.delta_link_threshold_start_position = None

				if self.delta_link_threshold_adjustment_factor is not None:
					self.delta_link_threshold *= self.delta_link_threshold_adjustment_factor

				# in case delta-link threshold < rd cutoff after manual adjustment (by adjustment factor)
				if self.delta_link_threshold < rd_cutoff:
					self.delta_link_threshold = rd_cutoff
			...
			return rd_cutoff, np.nanmax(distances)
	...
	def calculate_densities(self):
		...
		# self.normalized_densities = np.log(self.normalized_densities)	# base e
		if self.density_log_base is not None:
			self.normalized_densities = np.log10(self.normalized_densities) / np.log10(self.density_log_base)
		else:
			self.normalized_densities = np.log(self.normalized_densities)	# log base e
	...
	def calculate_information_gains(self, labels):
		...
		# entropy_calculation_method = getattr(self, 'entropy_calculation_method', None)
		...
		if labels is not None:
			...
			for i in range(self.reduced_num_data):
				...
				if i in self.reduced_known_data_labels:
					...
					if self.entropy_calculation_method == 'simple':
						pre_grading_entropy[i] = 0
					continue
		...
		if self.entropy_calculation_method == 'simple':
			information_gains = pre_grading_entropy
		else:
			information_gains = pre_grading_entropy - post_grading_entropy
		...
		if self.pgv_density_adjustment is not None:
			if self.pgv_density_adjustment == 'k':
				...
				if self.pgv_use_normalized_density:
					...
					adjustments = adjustments + self.densities	
		elif self.pgv_use_normalized_density:
			adjustments = self.normalized_densities
		else:
			adjustments = self.densities
		
		if self.entropy_density_weighting_method == 1:
			pgv_values = information_gains + adjustments
		elif self.entropy_calculation_method == 2:
			pgv_values = information_gains + (adjustments * 2)
		elif self.entropy_density_weighting_method == 3:
			pgv_values = information_gains * 2 + adjustments * 2
		else:
			pgv_values = information_gains * adjustments
		
		return information_gains, pgv_values
