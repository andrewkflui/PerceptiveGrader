...
class Answer(object):
	def __init__(self, answer_id, text, scores=None, is_reference=False, metadata=dict()):
		self.id = answer_id
		self.text = text
		self.scores = scores
	...
	def get_dominated_score(self):
		return max(self.scores, key=self.scores.count) if self.scores is not None else None
	...
	def get_answer_class(self, mapping, scale, dataset, question):
		...
		score_classes = question.metadata.get('score_classes') or dataset.metadata.get('score_classes')
...
# 			self.fixes_path = Path(config.ROOT_PATH) / 'data/datasets/fixes' / self.name
# 			self.grade_corrections_path = self.fixes_path / 'grade_corrections.csv'
# 		else:
# 			self.fixes_path = self.grade_corrections_path = None
...
class Dataset(object):
	...
	def __str__(self):
		string = '[DATASET]\n'
		string += 'NAME: {}{}\n'.format(self.name, f':{self.questions[0].id}' if len(self.questions) <= 1 else '')
		string += 'QUESTION COUNT: {}'.format(len(self.questions))
		return string

	def create_question_dataset(self, question_id):
		clone = self.__class__.__new__(self.__class__)
		for k, v in self.__dict__.items():
			if k == 'questions':
				continue
			setattr(clone, k, v)
		clone.questions = [self.get_question(question_id)]
		return clone

	def set_attributes(self, metadata):
	...
	def process(self, path):
		self.questions = []
		rows = utils.read_csv_file(path, delimiter=',')
		
		current_question_id = None
		for row in rows[1:]:
			question_id, question_text, answer_id, answer_text, is_reference = row
			if ':' in question_id:
				self.name, question_id = question_id.split(':')
			if current_question_id is None or question_id != current_question_id:
				self.questions.append(Question(question_id=question_id, domain=None, text=question_text))
				current_question_id = question_id
			self.questions[len(self.questions)-1].answers.append(Answer(answer_id=answer_id, text=answer_text,
				scores=None, is_reference=eval(is_reference)))

class PresetDataset(Dataset):
	def __init__(self, name, score_encoding, metadata, path):
		self.name = name
		self.metadata = metadata if metadata is not None else dict()
		if not 'has_reference_answer' in self.metadata:
			self.metadata['has_reference_answer'] = False
		self.metadata['score_encoding'] = score_encoding
		self.fixes_path = Path(config.ROOT_PATH) / 'data/datasets/fixes' / name
		self.grade_corrections_path = self.fixes_path / 'grade_corrections.csv'
		super().__init__(path, metadata=metadata)

	def __str__(self):
		string = '[PRESETDATASET]\n'
		string += 'NAME: {}\n'.format(self.name)
		string += 'HAS_REFERENCE_ANSWER: {}\n'.format(self.metadata.get('has_reference_answer'))
		string += 'SCORE CLASSES: {}\n'.format(self.metadata.get('score_classes'))
		string += 'SCORE ENCODING: {}\n'.format(self.metadata.get('score_encoding'))
...
class SciEntsBank2013Task7(PresetDataset):
	def __init__(self, subset_name, container_folder_name, score_classes, root_path):
		...
		# self.grade_corrections_path = self.fixes_path / 'grade_corrections.csv'
		super().__init__(name='SEB{}'.format(subset_name), score_encoding=ScoreEncoding.TEXT,
			metadata={'has_reference_answer': True, 'score_classes': score_classes}, path=root_path)
...
class USCIS(PresetDataset):
	def __init__(self, root_path, include_100=False):
		...
		# self.grade_corrections_path = self.fixes_path / 'grade_corrections.csv'
		super().__init__(name='USCIS{}'.format('_include_100' if include_100 else ''), score_encoding=ScoreEncoding.NUMERIC,
			metadata={'has_reference_answer': True, 'score_classes': [-1, 0, 1]}, path=root_path)
		...
		for row in rows:
			if i > 0:
				...
				if utils.is_empty_string(answer_text):
					logger.warning('Empty answer! question ID: {}, line: {}, answer_id: {}'.format(question.id, i, answer_id))
					continue
...
# class Mobley(PresetDataset):
class UNTComputerScienceShortAnswerDataset(PresetDataset):
	def __init__(self, root_path):
		# super().__init__(name='mobley', score_encoding=ScoreEncoding.NUMERIC,
		# 	metadata={'has_reference_answer': True}, path=root_path)
		super().__init__(name='unt', score_encoding=ScoreEncoding.NUMERIC,
			metadata={'has_reference_answer': True}, path=root_path)
...
class ChakrabortyAndKonar(PresetDataset):
	def __init__(self, root_path):
		super().__init__(name='cak', score_encoding=ScoreEncoding.NUMERIC, metadata=None, path=root_path)
...
# class ASAP(Dataset):
class ASAPAES(PresetDataset):
	def __init__(self, root_path):
		super().__init__('ASAPAES', ScoreEncoding.NUMERIC, metadata=None, path=root_path)
	...
	def parse_essay_detail_description(self, question, path):
		...
		while i < len(lines):
			...
			if not within_question_text and line == 'Prompt':
				within_question_text = True
				question.text = ''
			elif within_question_text and line.startswith('Rubric Guidelines'):
				within_question_text = False
			elif within_question_text:
				question.text += '{}\n'.format(line)
		...
		for i in range(sheet.nrows):
			...
			for j in range(sheet.ncols):
				...
				if i == 0:
					...
					question = Question(question_id=str(int(value)), domain='essay', text=None)
				else:
	...
	def parse_processed_essays(self, path):
		rows = utils.read_csv_file(Path('{}/processed/processed_dataset.csv'.format(path)), delimiter=',')
		headers = None
		...
		for row in rows:
			...
			answer_id, answer_text, question_id, score = row
			question = self.get_question(question_id)
			if question is None:
				continue
			question.answers.append(Answer(answer_id=answer_id, text=answer_text, scores=[float(score)]))

	def process(self, path):
		self.parse_essay_descriptions(path)
		# self.parse_training_set(path)
		self.parse_processed_essays(path)

class ASAPSAS(PresetDataset):
	def __init__(self, root_path):
		super().__init__('ASAPSAS', ScoreEncoding.NUMERIC, metadata=None, path=root_path)

	def process_questions(self, path):
		questions = []
		directory = Path(path) / 'Data_Set_Descriptions'
		for f in os.listdir(directory):
			if f.startswith('.') or f.startswith('~'):
				continue
			text = docx2txt.process(directory / f)
			lines = [line.strip() for line in text.split('\n') if not line.strip() == '']
			...
			while i < len(lines):
				...
				if line.startswith('Data Set #'):
					question = Question(question_id=line.replace('Data Set #', '').strip(), domain='', text='')
					question.metadata['description'] = ''
					i += 1
					continue

				if line.startswith('Rubric range'):
					i += 1
					line = lines[i]
					min_value, max_value = line.replace('Rubric range:', '').strip().replace(' ', '').split('-')
					question.metadata['score_classes'] = [str(i) for i in range(int(min_value), int(max_value) + 1)]
					continue

				if not within_question_text and (line.startswith('Prompt')):
					within_question_text = True
					question.text = ''
				elif within_question_text and (line.startswith('Rubric') or line.startswith('Scoring')):
					within_question_text = False
				elif within_question_text:
					question.text += '{}\n'.format(line)
				question.metadata['description'] += '{}\n'.format(line)
		...
		self.questions = sorted(questions, key=lambda q: int(q.id))

	def process_answers(self, path):
		rows = utils.read_csv_file(Path(path) / 'train.tsv', delimiter='\t')
		headers = None
		...
		for row in rows:
			...
			answer_id, question_id, score1, score2, answer_text = row
			question = self.get_question(question_id)
			if question is None:
				continue
			question.answers.append(Answer(answer_id=answer_id, text=answer_text, scores=[score1]))

	def process(self, path):
		self.process_questions(path)
		self.process_answers(path)
...
class SubDataset(object):
	def __init__(self, encoded_dataset, question, exclude_reference=True):
		...
		self.metadata.update(question.metadata)
	...
	def add_data(self, answers):
		if self.exclude_reference:
			self.answers += [answer for answer in answers if not answer.is_reference]
		else:
			self.answers += answers
		# self.answers = sorted(self.question.answers, key=lambda x: x.is_reference, reverse=True)

		self.reference_answers += [answer for answer in answers if answer.is_reference]
		self.reference_answer_data += [answer.encoding for answer in self.reference_answers]
		self.set_data([answer.encoding for answer in self.answers])
