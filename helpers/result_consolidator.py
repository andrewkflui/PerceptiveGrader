...
def consolidate_results_single_version(path, reorganize_folders=None, apply_file_count=10, algorithm=None):
	file_names = []
	...
	for file_paths in file_path_list:
		...
		for file_path in file_paths:
			v, s, d, r = process_excel_file_single_version(file_path, algorithm=algorithm)
			if descriptions is None:
				descriptions = d
			if results is None:
				results = [['Version', v]] if algorithm is None else [['Algorithm', algorithm]]
			results += r + [['']]
...
def process_excel_file_single_version(file_path, algorithm=None):
	version, random_seed, random_seed_header = None, None, 'Random Seed'
	...
	for row in worksheet.values:
		header = row[0].lower() if row[0] is not None else ''
		if header == 'random seed' or header == 'random state':
			random_seed = row[1]
			random_seed_header = row[0]
			result_started = header == 'random state'
		...
		elif (algorithm is None and header == 'subspaces') \
			or (algorithm == 'kmeans' and header == 'cluster num'):
			if not result_started:
				result_started = True
				results.append([random_seed_header, random_seed])
...
if __name__ == '__main__':
	...
	parser.add_argument('--algorithm', help='algorithm used', required=False)
	...
	consolidate_results_single_version(args.path, reorganize_folders=folders, algorithm=args.algorithm)
