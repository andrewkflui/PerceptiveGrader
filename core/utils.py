import os, csv, pickle, math, random, csv, json
...
class UnsupportedError(Exception):
	...
	def __str__(self):
		return f'{self.type.upper()} \"{self.name}\" IS NOT SUPPORTED ON THIS DEVICE'

class ParamNotSpecifiedError(Exception):
	def __init__(self, params, condition_string=None):
		self.params = params
		self.condition_string = condition_string

	def __str__(self):
		return 'OR '.join(self.params) + 'MUST BE PROVIDED'
...
class Worker(Thread):
	...
	def run(self):
		if self._target is not None:
			self._results = self._target(*self._args)
...
class MultiProcessingWrapper():
	...
	@staticmethod
	async def _run(queue, target, args=()):
		value = await target(*args)
		self._queue.put(value)

	def run_process(self, target, args=()):
		process = Process(target=self._run, args=[self._queue, target, args])
...
def plot(data, labels, title, core_data_indices=[], legend=True):
